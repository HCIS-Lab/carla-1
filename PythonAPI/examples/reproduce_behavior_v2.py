...
class World(object):
    ...
    def restart(self):
        ...
        blueprint = self.world.get_blueprint_library().filter('vehicle.*.*')[2]
...
def read_control(path='control.npy'):
    ...
    init_transform = carla.Transform(Location(x=control[0][0]+X, y=control[0][1]+Y, z=control[0][2]+Z),
...
def game_loop(args):
    ...
    try:
        ...
        for i in range(1, len(control_list)):
            clock.tick_busy_loop(60)
            world.player.apply_control(control_list[i])
