        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self._is_quit_shortcut(event.key):
                    ...
                    if_npc = True if input(
                        "NPC or player? ") == "NPC" else False
        ...
        if not self._parent.type_id.startswith("walker.pedestrian"):
            self._camera_transforms = [
                ...
                (carla.Transform(carla.Location(x=-1.0, y=-
                 1.0*bound_y, z=0.4*bound_z)), Attachment.Rigid),
        ...
        for item in self.sensors:
            ...
            if item[0].startswith('sensor.camera'):
                ...
                if bp.has_attribute('chromatic_aberration_intensity'):
                    bp.set_attribute(
                        'chromatic_aberration_intensity', str(3.0))
...
def scenario_tag_to_location(town):
    ...
    if town == "A1":
        ...
        for tags in region_tags:
            coords = region_tags[tags]
            x, y = 0, 0
            N = len(coords)
            for coor in coords:
                x += coor[0]
            ...
            intersection_coordinators[tags] = (x/N, y/N, 0)
