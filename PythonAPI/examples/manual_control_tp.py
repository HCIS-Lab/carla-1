...
Use ARROWS or WASD keys for control.
    ...
    J		 : teleport to a specific actor by its actor id.
    E		 : Save all actors' coordinate.
    K 		 : Teleport all actors to saved coordinate. 
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self._is_quit_shortcut(event.key):
                    ...
                    act_id = int(input("Actor ID: "))
                    tele_transform = world.world.get_actor(act_id).get_transform()
                    fv = tele_transform.get_forward_vector()
                    tele_transform.location.x += 5*fv.x
                    tele_transform.location.y += 5*fv.y
                    tele_transform.location.z += 5*fv.z
                    # tele_transform.location.z += 2
