...
def get_camera_intrinsic(VIEW_WIDTH,VIEW_HEIGHT,VIEW_FOV):
...
def world_to_sensor(cords, sensor):
    sensor_world_matrix = get_matrix(sensor)
...
def get_bounding_box(vehicle, VIEW_WIDTH,VIEW_HEIGHT,VIEW_FOV,s_transform):
    camera_k_matrix = get_camera_intrinsic(VIEW_WIDTH,VIEW_HEIGHT,VIEW_FOV)
    bb_cords = create_bb_points(vehicle)
    cords_x_y_z = vehicle_to_sensor(bb_cords, vehicle, s_transform)[:3, :]
...
def get_2d_bb(vehicle, VIEW_WIDTH,VIEW_HEIGHT,VIEW_FOV,s_transform):
    p3d_bb = get_bounding_box(vehicle, VIEW_WIDTH,VIEW_HEIGHT,VIEW_FOV,s_transform)
...
def get_list_transform(vehicles_list, s_transform):
    t_list = []
    ...
    sensor_world_matrix = get_matrix(s_transform)
    world_sensor_matrix = np.linalg.inv(sensor_world_matrix)
...
def filter_angle(vehicles_list, v_transform, v_transform_s, VIEW_FOV):
...
def filter_distance(vehicles_list, v_transform, v_transform_s, s, max_dist=100):
...
def filter_angle_distance(vehicles_list, sensor, VIEW_FOV,max_dist=100):
    ...
    vehicles_list , vehicles_transform , vehicles_transform_s = filter_angle(vehicles_list, vehicles_transform, vehicles_transform_s, VIEW_FOV)
