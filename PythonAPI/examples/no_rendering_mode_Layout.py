...
try:
    ...
    from pygame.locals import K_y # for saving maps
except ImportError:
...
# the color of walker
COLOR_PEOPLE = pygame.Color(255, 15, 57)

# the color of other car 
COLOR_OTHER_CAR = pygame.Color(255, 128, 0)

# the color of ego car 
COLOR_EGO_CAR = pygame.Color(47, 243, 224)

# the color of bike, motor
COLOR_Bike_MOTOR = pygame.Color(10, 112, 41)

# the color of other
COLOR_LEFT = pygame.Color(169, 96, 54)

# the color of interactive object 
COLOR_INTERACTIVE = pygame.Color(51, 51, 255)
...
PIXELS_AHEAD_VEHICLE = 0 # (2.8958897590637207+0.5)*0.8/(50/360) = 19.5603250122

#### test ####
shot = False
##############

import cv2  # for image resize and save
    ...
    def render_vehicles_ids(self, vehicle_id_surface, list_actors, world_to_pixel, hero_actor, hero_transform):
        ...
        if self.show_actor_ids:
            ...
            for actor in list_actors:
                ...
                print(actor[0].attributes['role_name'])
...
class MapImage(object):
    ...
    def __init__(self, carla_world, carla_map, pixels_per_meter, show_triggers, show_connections, show_spawn_points):
        ...
        dirname = os.path.join("cache", "Ground_Truth") # filepath
...
class World(object):
    ...
    def start(self, hud, input_control):
        ...
        rect_crop = pygame.Rect(center_offset[0] - int(self._hud.dim[0] / 4), 
        center_offset[1] - int(self._hud.dim[1] / 4), 
        int(self._hud.dim[0] / 2), int(self._hud.dim[1]) / 2)

        # rect_crop = pygame.Rect(center_offset[0] - int(self._hud.dim[0] / 2), 
        # center_offset[1] - int(self._hud.dim[1] / 2), 
        # int(self._hud.dim[0]), int(self._hud.dim[1]))
        #pygame.draw.rect(self.border_round_surface, COLOR_WHITE, rect_crop)

        # size of result image, important for size adjustment by Lun 2021.12.16
        scaled_original_size = self.original_surface_size * (1.8 / 0.9)
    ...
    def _render_walkers(self, surface, list_w, world_to_pixel):
        ...
        for w in list_w:
            # set walker color
            if w[0].attributes['role_name'] == 'tp':
    ...
    def _render_vehicles(self, surface, list_v, world_to_pixel):
        ...
        for v in list_v:
            ...
            if int(v[0].attributes['number_of_wheels']) == 2 and v[0].attributes['role_name'] == 'tp' :
                color = COLOR_INTERACTIVE
                # pygame.draw.polygon(surface, color, corners, 0)
                continue
            ...
            if int(v[0].attributes['number_of_wheels']) == 2 and v[0].attributes['role_name'] != 'tp' :
                color = COLOR_Bike_MOTOR
                #pygame.draw.lines(surface, color, False, corners, int(math.ceil(4.0 * self.map_image.scale)))
                # pygame.draw.polygon(surface, color, corners, 0)
                continue
            #  draw other cars
            if int(v[0].attributes['number_of_wheels']) != 2 and v[0].attributes['role_name'] != 'hero' and v[0].attributes['role_name'] != 'tp' :
                color = COLOR_OTHER_CAR
                # pygame.draw.polygon(surface, color, corners, 0)
                continue
            # draw ego car 
            if int(v[0].attributes['number_of_wheels']) != 2 and v[0].attributes['role_name'] == 'hero':
                color = COLOR_EGO_CAR
                #pygame.draw.polygon(surface, color, corners, 0)   
                continue
    ...
    def _compute_scale(self, scale_factor):
        ...
        self.map_image.scale_map(1.0)   # scaled 0.6 (720 pixel x 720 pixel, 30m x 30m -> 50m x 50m)
    ...
    def render(self, display):
        ...
        if self.hero_actor is not None:
            ...
            self.hero_surface.blit(self.result_surface, (-translation_offset[0],
                                                         -translation_offset[1]))
            rotated_result_surface = pygame.transform.rotate(self.hero_surface,angle).convert()
...
def game_loop(args):
    ...
    try:
        ...
        hud_height = min(world._hud.dim[0], world._hud.dim[1])
        #update_rect = ()
        # display.set_mode(
        #     (hud_height/2, hud_height/2),
        #     pygame.HWSURFACE | pygame.DOUBLEBUF)
        new_rect = (args.width/2 - args.height/2 , 0, args.height, args.height)

        # color_mapping = {'other_cars': [92, 53, 102], 'non-drivable_area': [85, 87, 83], 'side_walk': [136, 138, 133],\
        #     'drivable_lae': [46, 52, 54], 'cross_walk': [116, 191, 101], 'shoulder': [66, 62, 64],\
        #         'white_solid_lane': [255, 58, 58], 'white_broken_lane': [58, 248, 255], 'yelow_solid_lane': [252, 175, 62],\
        #             'yellow_broken_lane': [122,100,67]}
        data_root = './data_collection/{}/{}/{}_{}_/map/Layout'.format(args.scenario_type, args.scenario_id , args.weather, args.random_actors)
        ...
        while True:
            ...
            global shot
            if shot == True:
                #new_rect = (world._hud.dim[0]/2 - hud_height/4 ,world._hud.dim[1]/2 - hud_height/4, hud_height/2, hud_height/2)
                new_surface = pygame.Surface.subsurface(display, new_rect).convert()
                pygame.image.save(new_surface, 'meow')
                pygame.image.save(pygame.transform.scale(new_surface,(200,200)), 'meow200')
                view = pygame.surfarray.array3d(new_surface)
                ...
                img_bgr_resize = cv2.resize(img_bgr, (200, 200), interpolation=cv2.INTER_AREA) #INTER_AREA, INTER_NEAREST
                cv2.imwrite('meowcv.png', img_bgr_resize, [cv2.IMWRITE_PNG_COMPRESSION, 0])
                

                color_semantic = []
                for x in range(200):
                    for y in range(200):
                        color = img_bgr_resize[x][y]
                        have = False
                        for color_in_list in color_semantic:
                            if color[0] == color_in_list[0][0]\
                                and color[1] == color_in_list[0][1]\
                                and color[2] == color_in_list[0][2]:
                                have = True
                                color_in_list[1] += 1
                                break
                        if not have:
                            color_semantic.append([color, 1])
                            print(color)
                print(len(color_semantic))

                shot = False

            new_surface = pygame.Surface.subsurface(display, new_rect).convert()
            try:
                data_path = data_root + '/%08d.png'%(world.world.wait_for_tick().frame)
                pygame.image.save(new_surface, data_path)
            except:
                break
