...
class World(object):
    def __init__(self, carla_world, hud, args,location=None):
    ...
    def restart(self,location=None):
    ...
    def toggle_recording(self):
        ...
        if not self.recording:
            for img in self.record_image:
                img.save_to_disk('_out/%s/%08d' % (self.sensors[self.index][2],img.frame))
            self.record_image=[]
...
def game_loop(args):
    ...
    try:
        ...
        location=[0,0,0]
        location[0]=int(input("x: "))
        location[1]=int(input("y: "))
        location[2]=int(input("z: "))
        world = World(client.get_world(), hud, args,location)
