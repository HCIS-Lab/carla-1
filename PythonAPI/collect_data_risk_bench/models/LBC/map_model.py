...
from models.LBC.models import SegmentationModel, RawController
from models.LBC.utils.heatmap import ToHeatmap
import models.LBC.common as common
from models.LBC.dataset import get_dataset
...
def visualize(batch, out, between, out_cmd, loss_point, loss_cmd, target_heatmap):
    ...
    for i in range(out.shape[0]):
        ...
        topdown, points, target, actions = [x[i] for x in batch]
...
class MapModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_nb):
        topdown, points, target, actions = batch
    ...
    def train_dataloader(self):
        return get_dataset(self.hparams, is_train=True)

    def val_dataloader(self):
        return get_dataset(self.hparams, is_train=False)

def main(hparams):
    model = MapModel(hparams)
    logger = WandbLogger(id=hparams.id, save_dir=str(hparams.save_dir), project='LBC-Privileged Model')
    ...
    try:
        ...
        # e35abdf8459a44fba221b0d18bd20b51
...
if __name__ == '__main__':
    ...
    # Data args.
    parser.add_argument('--dataset_dir', default="/media/chiehchiyang/WD/LBC/dataset", type=pathlib.Path) #, required=True)
