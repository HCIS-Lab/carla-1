...
class custom_loss(nn.Module):
    def __init__(self, time, weights, supervised=False):
        ...
        if supervised:
            self.obj_cel =  nn.CrossEntropyLoss(reduction = 'mean')
    ...
    def forward(self, outputs, targets, obj_pred=None, obj_targets=None):
        ...
        for i,pred in enumerate(outputs):
            ...
            if self.supervised:
                obj_loss = self.obj_cel(obj_pred[:,i],obj_targets[:,i])
                print("Loss:",obj_loss, loss)
                loss = loss + obj_loss
        return loss

class Baseline_SA(nn.Module):
    def __init__(self,backbone,n_frame, object_num, img_features_size=256, hidden_layer_size=256, lstm_size=512,supervised=False,zeros_object=False,intention=False):
        ...
        if supervised:
            self.supervised_obj = nn.Linear(hidden_layer_size,object_num+1)

        self.features_size = img_features_size
        self.zeros_object = zeros_object
        self.lstm_layer_size = lstm_size

        self.object_layer = nn.Linear(img_features_size,hidden_layer_size)
    ...
    def forward(self, img, bbox,intention=None):
        """
            img: b t 3 H W
        """
        device = img.device
        ...
        for i in range(frames):
            ...
            object = object.reshape(-1, self.features_size).contiguous() # (n b) c
            object = self.object_layer(object) #(nxb)xh
            object = object.reshape(self.object_num,batch_size,-1)
            if self.zeros_object:
                object = object*torch.unsqueeze(zeros_object[i],2)
            
            # object attention
            _object = self.object_attention(object)
            alphas = self.to_attention(self.h_prev(hx) + _object).softmax(dim=0) # n b 1
            if self.zeros_object:
                alphas = alphas * zeros_object[i]
            attention_list = alphas * object # n b h
            attention = attention_list.sum(0) # b h
            if self.supervised:
                to_obj = attention.clone()
                obj_pred = self.supervised_obj(to_obj)
            # concat frame & object
            if intention is not None:
        ...
        if self.supervised:
            all_obj = torch.stack(all_obj).permute(1,0,2)
